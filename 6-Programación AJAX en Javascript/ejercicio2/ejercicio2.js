...
function cargarDatos() {
  ...
  xhr.send();
        ...
        let datos = docXML.getElementsByTagName("disco");
        for (let i = 0; i < datos.length; i++) {
        let nombre = datos[i].getElementsByTagName("nombre")[0].textContent;
        let componentes = datos[i].getElementsByTagName("grupo")[0].textContent;         
        let publicacion = datos[i].getElementsByTagName("year")[0].textContent;         
        let tipoMusica = datos[i].getElementsByTagName("tipo")[0].textContent;
        let localizacion = datos[i].getElementsByTagName("localizacion")[0].textContent;          
        let prestado = datos[i].getElementsByTagName("prestado")[0].textContent;        
        let caratula = datos[i].getElementsByTagName("caratula")[0].textContent;
        discos.push(new Disco(nombre,componentes,publicacion,tipoMusica,localizacion,prestado,caratula));
           }
